...
```
docker build -t ghumman/react-frontend:latest .
```

If you want to run that image locally:
```
docker run -p 8080:8080 ghumman/react-frontend:latest
```

If you want to push this docker image to docker hub:
```
docker push ghumman/react-frontend:latest
```


### Note

- This branch is not used to push to github pages. Use branch `react-js` to push to github pages. This branch is used to create docker image and push it to docker hub. 
